...
template <typename... Ts>
inline std::string format(const char *Fmt, const Ts &... Vals) {
  auto fmt = format_object<Ts...>(Fmt, Vals...);
  size_t MaxBufferSize = 384;
  std::vector<char> V(MaxBufferSize);
  size_t byteUsed = fmt.print(V.data(), MaxBufferSize);
  // TODO we didn't have enough space in the output buffer for the string
  assert(byteUsed <= MaxBufferSize);
  return std::string(V.data(), byteUsed);
}
