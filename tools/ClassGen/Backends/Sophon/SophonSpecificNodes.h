...
BB.newNode("SophonMaxPoolQ8")
    ...
    .addMember(MemberType::Boolean, "RoundMode")
    .addResultFromCtorArg()
    .setDocstring("Performs a Max Pool int8 operation on the Input given provided "
                  "Kernels, Strides, and Pads.");
...
BB.newNode("SophonAvgPoolQ8")
    ...
    .addMember(MemberType::Boolean, "RoundMode")
    .addResultFromCtorArg()
    .setDocstring("Performs a Avg Pool int8 operation on the Input given provided "
                  "Kernels, Strides, and Pads.");
...
BB.newNode("SophonFullyConnectedQ8")
    ...
    .addResultFromCtorArg()
    .setDocstring("Creates a FullyConnected int8 node where the Input tensor and "
                  "Weights tensor are multiplied, and then the Bias tensor "
                  "is added to it, producing the Output.");
...
BB.newNode("SophonMatMul")
    ...
    .addInput("Slice")
    .addResultFromCtorArg()
    .setDocstring("Performs matrix multiplication between the LHS RHS, and "
...
BB.newNode("SophonLocalResponseNormalization")
    ...
    //.addMember(MemberType::Float, "bias") caffe doesn't have this parameter, comment it
...
BB.newNode("SophonDeconvolution")
    ...
    .addMember(MemberType::Unsigned, "Group")
    .addResultFromCtorArg()
    .setDocstring("Performs Deconvolution using a given Input, Filter, and "
...
BB.newNode("SophonROIPool")
    ...
    .addResultFromCtorArg()
    .setDocstring("ROI pool consumes an input tensor X and region of interests "
        "(RoIs) to apply pooling across each RoI, to produce output 4-D tensor "
        "of shape (num_rois, channels, pooled_shape[0], pooled_shape[1]). Similar "
...
BB.newNode("SophonPSROIPool")
    ...
    .addMember(MemberType::Unsigned, "Group")
    .addResultFromCtorArg()
    .setDocstring("Position sensitive ROI pooling. Similar to RFCN PSROIPooling.");
...
BB.newNode("SophonEltwise")
    ...
    .addResultFromCtorArg()
    .setDocstring("The eltwise operator adds/product/max two tensors together.");
...
BB.newNode("SophonYolo")
    .addInput("Input")
    .addMember(MemberType::Unsigned, "classes") //by ycs: in bmnet-caffe.proto, it's int32? not usigned?
    .addResultFromCtorArg()
    .setDocstring("yolo.");
...
//    .addMember(MemberType::VectorFloat, "scale") by ycs: cann't use vectorfloat yet, need fix
//    .addMember(MemberType::VectorFloat, "ratio")
    .addResultFromCtorArg()
    .setDocstring("Prosal.");
