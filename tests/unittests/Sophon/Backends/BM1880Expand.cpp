...
TEST(BM1880ExpandTest, ExpandConvQ8) {
  ...
  // N, C, H, W
  auto *input = bb.createWeightVar(glow::ElemKind::Int8QTy, {1, 1, 3, 3}, 1, 0, "input",
                                   WeightVar::MutabilityKind::Mutable);

  // IC, OC, KH, KW
  auto *conv_w = bb.createWeightVar(glow::ElemKind::Int8QTy, {3, 1, 3, 3}, 1, 0, "conv1.w",
                                    WeightVar::MutabilityKind::Constant);

  // OC
  auto *conv_b = bb.createWeightVar(glow::ElemKind::Int16QTy, {3}, 1, 0, "conv1.b",
                                    WeightVar::MutabilityKind::Constant);

  // N, C, H, W
  auto *output = bb.createWeightVar(glow::ElemKind::Int8QTy, {1, 3, 1, 1}, 1, 0, "output1",
                                    WeightVar::MutabilityKind::Mutable);

  auto *lmem_in = bb.createAllocActivationInst("lmem.in", input->getType());
  auto *lmem_weight = bb.createAllocActivationInst("lmem.weight", conv_w->getType());
  auto *lmem_bias = bb.createAllocActivationInst("lmem.bias", conv_b->getType());
  ...
  bb.createSophonConvolutionQ8Inst("conv1", lmem_out, lmem_in, lmem_weight, lmem_bias,
                                   {stride_h, stride_w}, {pad_top, pad_left, pad_bottom, pad_right},
                                   {dilation_h, dilation_w}, rshift_width, enable_relu);
  ...
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIConvolutionQ8InstKind);
  ...
  EXPECT_EQ(MI_inst->getStrideHW(), (llvm::ArrayRef<unsigned>{stride_h, stride_w}));
  EXPECT_EQ(MI_inst->getPadTLBR(),
            (llvm::ArrayRef<unsigned>{pad_top, pad_left, pad_bottom, pad_right}));
  EXPECT_EQ(MI_inst->getDilationHW(), (llvm::ArrayRef<unsigned>{dilation_h, dilation_w}));
...
TEST(BM1880ExpandTest, ExpandMaxPoolingQ8) {
  ...
  // N, C, H, W
  auto *input = bb.createWeightVar(glow::ElemKind::Int8QTy, {1, 1, 4, 4}, 1, 0, "input",
                                   WeightVar::MutabilityKind::Mutable);

  // N, C, H, W
  auto *output = bb.createWeightVar(glow::ElemKind::Int8QTy, {1, 1, 2, 2}, 1, 0, "output1",
                                    WeightVar::MutabilityKind::Mutable);
  ...
  bb.createSophonMaxPoolQ8Inst("pool", lmem_out, lmem_in, {kernel_h, kernel_w},
                               {stride_h, stride_w}, {pad_top, pad_left, pad_bottom, pad_right},
                               rshift_width, multiplier);
  ...
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIMaxPoolingQ8InstKind);
  ...
  EXPECT_EQ(pool_inst->getKernelHW(), (llvm::ArrayRef<unsigned>{kernel_h, kernel_w}));
  EXPECT_EQ(pool_inst->getStrideHW(), (llvm::ArrayRef<unsigned>{stride_h, stride_w}));
  EXPECT_EQ(pool_inst->getPadTLBR(),
            (llvm::ArrayRef<unsigned>{pad_top, pad_left, pad_bottom, pad_right}));
  std::advance(cur_inst, 1);
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIMulConstQ8InstKind);
...
TEST(BM1880ExpandTest, ExpandReluQ8) {
  ...
  // N, C, H, W
  auto *output = bb.createWeightVar(glow::ElemKind::Int8QTy, {1, 1, 4, 4}, 1, 0, "output1",
                                    WeightVar::MutabilityKind::Mutable);
...
TEST(BM1880ExpandTest, ExpandFcQ8) {
  ...
  // in_row * in_col
  auto *input = bb.createWeightVar(glow::ElemKind::Int8QTy, {3, 4}, 1, 0, "input",
                                   WeightVar::MutabilityKind::Mutable);

  // in_col * out_col
  auto *weight = bb.createWeightVar(glow::ElemKind::Int8QTy, {4, 2}, 1, 0, "weight",
                                    WeightVar::MutabilityKind::Constant);
  ...
  auto *bias = bb.createWeightVar(glow::ElemKind::Int16QTy, {2}, 1, 0, "bias",
                                  WeightVar::MutabilityKind::Constant);
  // in_row * out_col
  auto *output = bb.createWeightVar(glow::ElemKind::Int8QTy, {3, 2}, 1, 0, "output1",
                                    WeightVar::MutabilityKind::Mutable);

  auto *lmem_in = bb.createAllocActivationInst("lmem.in", input->getType());
  auto *lmem_weights = bb.createAllocActivationInst("lmem.weights", weight->getType());
  ...
  bb.createSophonFullyConnectedQ8Inst("fc", lmem_out, lmem_in, lmem_weights, lmem_bias, relu,
                                      rshift_width, lshift_width, result_add);
...
TEST(BM1880ExpandTest, ExpandLoadQ8) {
  ...
  auto *input = mod.createPlaceholder(ElemKind::Int8QTy, {1, 1, 3, 3}, 1, 0, "input", false);
  auto *filter = mod.createConstant(ElemKind::Int8QTy, {3, 1, 3, 3}, 1, 0, "filter");
  auto *bias = mod.createConstant(ElemKind::Int16QTy, {3}, 1, 0, "bias");
  auto output_type = mod.uniqueType(ElemKind::Int8QTy, {1, 3, 3, 3}, 1, 0);

  auto *conv = F->addNode(new SophonConvolutionQ8Node("conv", output_type, input, filter, bias,
  ...
  auto cur_inst = IR->getInstrs().begin();
  std::advance(cur_inst, 2);
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIGDMAGlobalToLocalInstKind);
  ...
  EXPECT_EQ(load_input->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{1 * 3 * 3, 3 * 3, 3}));
  ...
  EXPECT_EQ(load_weight->getShapeNCHW(), (llvm::ArrayRef<unsigned>{1, 3, 9, 1}));
  EXPECT_EQ(load_weight->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{3 * 9, 9, 1}));
  ...
  EXPECT_EQ(load_bias->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{3 * 1 * 1, 1 * 1, 1}));
  EXPECT_EQ(load_bias->getIsGlobalWeightSpace(), true);
  EXPECT_EQ(load_bias->getIsLocalAligned(), false);

  std::advance(cur_inst, 2);
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIGDMALocalToGlobalInstKind);
  auto *store = llvm::cast<SophonMIGDMALocalToGlobalInst>(cur_inst);
  EXPECT_EQ(store->getShapeNCHW(), (llvm::ArrayRef<unsigned>{1, 3, 3, 3}));
  EXPECT_EQ(store->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{3 * 3 * 3, 3 * 3, 3}));
...
TEST(BM1880ExpandTest, ExpandLoadStoreFcQ8) {
  ...
  auto *input = mod.createPlaceholder(glow::ElemKind::Int8QTy, {1, 800}, 1, 0, "input", false);

  auto *fc_right = mod.createConstant(glow::ElemKind::Int8QTy, {800, 500}, 1, 0, "fc_right");

  // bias
  auto *fc_b = mod.createConstant(glow::ElemKind::Int16QTy, {500}, 1, 0, "fc.b");

  TypeRef ty = mod.uniqueType(glow::ElemKind::Int8QTy, {1, 500}, 1, 0);
  auto *fc = F->addNode(
      new SophonFullyConnectedQ8Node("fc", ty, input, fc_right, fc_b, false, 1, 2, false));
  ...
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIGDMAGlobalToLocalInstKind);
  auto *load_input = llvm::cast<SophonMIGDMAGlobalToLocalInst>(cur_inst);
  EXPECT_EQ(load_input->getShapeNCHW(), (llvm::ArrayRef<unsigned>{1, 25, 1, 32}));  // 25=800/32
  EXPECT_EQ(load_input->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{25 * 1 * 32, 1 * 32, 32}));
  EXPECT_EQ(load_input->getIsGlobalWeightSpace(), false);
  EXPECT_EQ(load_input->getIsLocalAligned(), true);
  ...
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIGDMAGlobalToLocalInstKind);
  auto *load_weight = llvm::cast<SophonMIGDMAGlobalToLocalInst>(cur_inst);
  EXPECT_EQ(load_weight->getShapeNCHW(), (llvm::ArrayRef<unsigned>{800, 16, 1, 32}));  // 16=500/32
  EXPECT_EQ(load_weight->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{500, 1 * 32, 32}));
  EXPECT_EQ(load_weight->getIsGlobalWeightSpace(), true);
  ...
  EXPECT_EQ(cur_inst->getKind(), glow::Kinded::Kind::SophonMIGDMAGlobalToLocalInstKind);
  auto *load_bias = llvm::cast<SophonMIGDMAGlobalToLocalInst>(cur_inst);
  EXPECT_EQ(load_bias->getShapeNCHW(), (llvm::ArrayRef<unsigned>{2, 16, 1, 32}));  // 16=500/32
  EXPECT_EQ(load_bias->getGlobalStrideNCH(), (llvm::ArrayRef<unsigned>{500, 1 * 32, 32}));
  EXPECT_EQ(load_bias->getIsGlobalWeightSpace(), true);
  EXPECT_EQ(load_bias->getIsLocalAligned(), false);
