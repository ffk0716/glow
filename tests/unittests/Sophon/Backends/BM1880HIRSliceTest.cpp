...
void sliceData(SophonConvolutionQ8Node *node, unsigned oh_max_slice) {
  ...
  for (int sec = 0; sec < ohsec; sec++) {
    ...
    // Make sure idx >= 0
    unsigned ih_idx = std::max(0, static_cast<int>(oh_idx * stride_h - pad_begin));
    ...
    auto *node_input = F->addNode(new SliceNode(input_name, type_islice, input, {0, 0, ih_idx, 0}));
    ...
    auto *node_conv = F->addNode(
        new SophonConvolutionQ8Node(node->getName(), type_output, node_input, nodevalue_filter,
...
TEST(BM1880HIRSliceTest, ConvQ8SliceData) {
  ...
  SophonConvolutionQ8Node *node_conv;
  {
    ...
    node_conv = F->addNode(new SophonConvolutionQ8Node("conv.1", type_output, node_input,
                                                       node_filter, node_bias, stride_hw, pad_tlbr,
                                                       dilation_hw, rshift_width, enable_relu));
  }
  {
    ...
    F->addNode(new SophonMaxPoolQ8Node("maxpool.1", type_output, node_conv, kernel_hw, stride_hw,
                                       pad_tlbr, rshift, mul, true));
  }
