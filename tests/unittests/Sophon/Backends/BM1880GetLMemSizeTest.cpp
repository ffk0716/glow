...
TEST(BM1880GetLMemSizeTest, LIRConv) {
  ...
  auto *lmem_bias = bb.createAllocActivationInst("bias", q16({64}));  // <2, 64, 1, 1>
  ...
  EXPECT_EQ(sz_in, 16 * 2);          // channel size = 3*3 aligned to 16, 2 channels per lane
  EXPECT_EQ(sz_weight, 64 * 3 * 3);  // channel size = 3*3*64, 1 channel per lane
  EXPECT_EQ(sz_out, 16 * 1);         // channel size = 3*3 aligned to 16, 1 channel per lane
...
TEST(BM1880GetLMemSizeTest, LIRFC) {
  ...
  auto *lmem_L = bb.createAllocActivationInst("matrix.L", q8({1, 800}));    // <1, 26, 1, 32>
  auto *lmem_R = bb.createAllocActivationInst("matrix.R", q8({800, 500}));  // <800, 16, 1, 32>
  auto *lmem_Y = bb.createAllocActivationInst("matrix.Y", q8({1, 500}));    // <1, 16, 1, 32>
  auto *lmem_bias = bb.createAllocActivationInst("bias", q16({500}));       // <2, 16, 1, 32>

  bool enable_relu = false;
  int shift_width = 0;
  bool result_add = false;

  bb.createSophonFullyConnectedQ8Inst("fc1", lmem_Y, lmem_L, lmem_R, lmem_bias, enable_relu,
                                      shift_width, shift_width, result_add);
  ...
  EXPECT_EQ(sz_R, 800 * 1 * 32);  // channel size = 800 * 1 * 32, one channel per lane
