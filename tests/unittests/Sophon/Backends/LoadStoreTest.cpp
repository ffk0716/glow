...
class LoadStoreTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    auto *input = mod.createPlaceholder(ElemKind::FloatTy, {1, 32, 32, 3}, "input", false);
    ElemKind inputTy = input->getType()->getElementType();
    auto *filter = mod.createPlaceholder(inputTy, {16, 5, 5, 3}, "filter", true);
    auto *bias = mod.createPlaceholder(inputTy, {16}, "bias", true);
    auto OT = mod.uniqueType(inputTy, {1, 32, 32, 16});

    auto *C = F->addNode(new SophonConvolutionQ8Node("conv", OT, input, filter, bias, {1, 1},
                                                     {2, 2, 2, 2}, {0, 0}, 0, false));
  ...
  void TearDown() override {
    ...
    IRBuilder B(IR.get());  // parser Function again for finding missing Inst, ex. deallocactivation
